...
import com.frogparser.flow.domain.command.function.family.string.StrStartsWithFunctionCommand;
...
@Service
public class StrStartsWithFunctionCommandExecutorService extends AbstractCommandExecutorService<StrStartsWithFunctionCommand> {
    @Override
    public void execute(FlowExecutionSettings flowExecutionSettings, FlowContext flowContext, StrStartsWithFunctionCommand command) throws CommandExecutionException {
        flowContext.getVariables().put(getNotBlankVariableName(command.getResult()), new BooleanRunTimeVariable(
                getRunTimeVariableValue(flowContext, command.getParameterOne(), StringRunTimeVariable.class)
                        .startsWith(getRunTimeVariableValue(flowContext, command.getParameterTwo(), StringRunTimeVariable.class))
        ));
    }
}
